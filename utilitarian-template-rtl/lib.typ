// Utilitarian Template (RTL Variant)
// A function-first, clean, and direct document template for Persian/RTL languages
...
#let mono-font = ("Vazir Code FD", "JetBrains Mono", "Consolas", "Courier New")
#let body-font = ("Sahel FD", "Vazirmatn", "Inter", "Roboto", "Arial")
...
#let util-field(label, value) = {
  grid(
    ...
    gutter: 1em,
    align: horizon,
    text(font: mono-font, size: 8pt, fill: util-slate, tracking: 0pt, baseline: -0.75pt, upper(label)),
...
#let util-code(lang: none, code) = {
  block(
    ...
    fill: util-warm-gray,
    radius: 0pt,
    text(font: mono-font, size: 9pt, fill: util-charcoal, dir: ltr, code)
  )
}
...
#let util-note(body) = {
  block(
    width: 100%,
    inset: (right: 12pt, y: 8pt, left: 8pt),
    stroke: (right: 2pt + util-accent),
...
#let utilitarian-doc(
    ...
    header: context {
      if counter(page).get().first() > 1 {
        grid(
          columns: (1fr, auto),
          align: (start, end),
          text(font: mono-font, size: 7pt, fill: util-slate, tracking: 0pt, upper(title)),
  ...
  // Heading styles - Uppercase, monospaced
  ...
  show raw.where(block: false): it => {
    box(
      ...
      inset: (x: 3pt, y: 1pt),
      radius: 1pt,
      text(font: mono-font, size: 9pt, dir: ltr, it)
    )
  }
  ...
  show raw.where(block: true): it => {
    block(
      ...
      inset: 12pt,
      radius: 0pt,
      text(font: mono-font, size: 9pt, fill: util-charcoal, dir: ltr, it)
    )
  }
  
  // Figure styling
  show figure.caption: it => {
    text(font: mono-font, size: 8pt, fill: util-slate, tracking: 0pt, upper(it))
  }
  ...
  // Minimal grid overlay visual - Flipped for RTL
  place(
    top + left,
    dx: 2cm,
    dy: 2cm,
  ...
  if document-id != none {
    text(font: mono-font, size: 8pt, fill: util-slate, tracking: 0pt, upper(document-id))
    v(0.5cm)
  }
  ...
  if author != none or date != none or version != none {
    grid(
      columns: (1fr, 1fr),
      row-gutter: 0.8em,
      if author != none { util-field("نویسنده", author) },
      if version != none { util-field("نسخه", version) },
      if date != none { util-field("تاریخ", date) } else { util-field("تاریخ", datetime.today().display("[year]-[month]-[day]")) },
      [],
    )
  }
  ...
  if toc {
    heading(outlined: false, numbering: none, 
      text(font: mono-font, size: 14pt, weight: "bold", fill: util-black, tracking: 0pt, "فهرست مطالب")
    )
    v(1cm)
