...
// Paragraph setup - line spacing matching LaTeX \doublespacing
#set par(
  leading: 0.65em,  // Approximates LaTeX \doublespacing (about 1.65× line height)
  justify: true,
...
پیچیدگی زمانی الگوریتم $O(n log n + n dot t(d))$ است. $t(d)$ پیچیدگی زمانی محاسبه فاصله $d(p,I)$ را نشان می‌دهد. محاسبه این فاصله با پیچیدگی زمانی $O(log^2 n)$ قابل انجام است. بنابراین پیچیدگی نهایی الگوریتم، $O(n log^2 n)$ است که در ادامه با استفاده از تکنیک جاروی صفحه، بهبود می‌یابد.
